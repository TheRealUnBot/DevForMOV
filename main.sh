...
case $ans in
  ...
  5 ) git init; git config --global user.name "TheRealUnBot"; git config --global user.email "161210@student.vansd.org"; git add -A; read -p "Commit Message:" mess; git commit -m $mess; git remote add origin https://TheRealUnBot:$KEY_GIT@github.com/TheRealUnBot/MinecraftOfVPS.git; git push -u -f origin main; exit;;
