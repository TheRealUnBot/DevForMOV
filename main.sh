...
case $ans in
  ...
  5 ) git config --global user.email "161210@student.vansd.org"; git config --global user.name "TheRealUnBot"; git init; git add .; git commit -m 'test'; git remote set-url origin https://github.com/TheRealUnBot/DevForMOV.git; git push -u origin main; exit;;
esac